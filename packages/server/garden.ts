...
interface ChallengeRow {id:string;actor_id:string;version:number;state:GardenChallenge['state'];timezone:string;policy_version:string;boundaries:string[];seed:string|number;created_at:Date|string;archived_at:Date|string|null}
const iso=(v:Date|string)=>new Date(v).toISOString();
function descriptor(c:ChallengeRow,grownWeeks:number,archivedAt:string|null=null):PlantInstance {
 const shape=generatePlant(Number(c.seed));return {id:c.id,seed:shape.seed,species:shape.species,generatorVersion:shape.generatorVersion,rendererVersion:shape.rendererVersion,grownWeeks,archivedAt};
}
