...
export function PlantLite({seed,weeks,height=140}:{seed:number;weeks:number;height?:number}){const p=useMemo(()=>generatePlant(seed>>>0),[seed]);return <Svg width="100%" height={height} viewBox="0 0 240 300" accessible={false}><Defs><SVGGradient id="pot" x1="0" y1="0" x2="1" y2="0"><Stop offset="0" stopColor="#E3E7DC" stopOpacity={.6}/><Stop offset="0.2" stopColor="#9DA99E" stopOpacity={.12}/><Stop offset="0.8" stopColor="#9DA99E" stopOpacity={.14}/><Stop offset="1" stopColor="#E8D6B9" stopOpacity={.6}/></SVGGradient><SVGGradient id="leaf" x1="0" y1="0" x2="1" y2="1"><Stop offset="0" stopColor="#BECA68"/><Stop offset="0.5" stopColor="#607329"/><Stop offset="1" stopColor="#283C17"/></SVGGradient></Defs><Path d="M77 200 Q60 252 87 270 Q120 282 153 270 Q180 252 163 200Z" fill="#1B1910"/><G>{p.leaves.filter(d=>leafGrowth(d,weeks)>0).map((d,i)=>{const g=leafGrowth(d,weeks);const x=120+Math.sin(d.angle)*42,y=196-d.height*75,l=d.length*95*g,side=Math.sin(d.angle)>=0?1:-1;return <G key={d.id}><Path d={`M120 205 Q${x} ${y+20} ${x} ${y}`} stroke="#72893D" strokeWidth={2.2} fill="none"/><Path d={`M${x} ${y} Q${x+side*l*.95} ${y+10} ${x+side*l*.65} ${y-l} Q${x-side*l*.25} ${y-l*.68} ${x} ${y}Z`} fill="url(#leaf)" stroke="#B4C681" strokeWidth={.6}/><Path d={`M${x} ${y} Q${x+side*l*.30} ${y-l*.55} ${x+side*l*.65} ${y-l}`} stroke="#CAD099" strokeWidth={.8} fill="none"/></G>})}</G>{weeks===0&&<Ellipse cx={120} cy={194} rx={5} ry={8} fill="#947349"/>}<Path d="M77 200 Q60 252 87 270 Q120 282 153 270 Q180 252 163 200Z" fill="url(#pot)" stroke="#CAD2C9" strokeWidth={1.2}/><Ellipse cx={120} cy={200} rx={43} ry={7} fill="none" stroke="#CAD2C9" strokeWidth={1.2}/></Svg>}
