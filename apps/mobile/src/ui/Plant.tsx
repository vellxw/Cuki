...
function leafTexture(seed:number){const width=128,height=256,data=new Uint8Array(width*height*4);for(let y=0;y<height;y++)for(let x=0;x<width;x++){const u=x/(width-1),v=y/(height-1),center=Math.abs(u-.5),rib=Math.abs(Math.sin(v*48+center*26));const mottled=Math.sin(u*38+seed%37)*Math.sin(v*55+Math.sin(u*23))*Math.sin((u+v)*31);const pale=mottled>.15&&center<.4;const vein=center<.012||rib<.055;const n=vein?1:pale?.64:.17;const i=(y*width+x)*4;data[i]=28+120*n;data[i+1]=55+104*n;data[i+2]=13+47*n;data[i+3]=255}const t=new THREE.DataTexture(data,width,height,THREE.RGBAFormat);t.colorSpace=THREE.SRGBColorSpace;t.needsUpdate=true;return t}
...
export function Plant({seed,weeks,height=300}:{seed:number;weeks:number;height?:number}) {
 ...
 return <View style={{height,width:'100%'}} accessibilityLabel={`Planta procedural, ${week} semanas acreditadas`} accessible>
  <PlantSurface key={seed} active={active} fallback={alternative}>{onFirstDraw=><Suspense fallback={null}>
   <Canvas style={{width:'100%',height:'100%'}} frameloop={!active?'never':reduceMotion?'demand':'always'}
    ...
    <directionalLight position={[2,4,3]} intensity={3.2} color="#FFF0D5"/>
    <directionalLight position={[-3,1,2]} intensity={1.1} color="#B4D7CE"/>
    <Object3D descriptor={descriptor} week={week} animated={active&&!reduceMotion} onFirstDraw={onFirstDraw}/>
   </Canvas>
